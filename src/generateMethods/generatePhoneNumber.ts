...
export const generatePhoneNumber = (person: Person | null) => {
    ...
    const startDigits = ["2", "30", "31", "40", "41", "42", "50", "51", "52", "53", "60", "61", "71", "81", "91", "92", "93", 
    ...
    const randomIndex = generateRandomNumber(0, startDigits.length);
    const randomStartDigit = startDigits[randomIndex];
    randomPhoneNumber = randomStartDigit;
    for (let index = 0; index < phoneNumberSize - randomStartDigit.length; index++) {
        randomPhoneNumber += generateRandomNumber(0, 10).toString();
